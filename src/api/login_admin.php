...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $query = "SELECT * FROM administrateurs WHERE login = ? AND statut = 'actif'";
        ...
        if ($stmt->rowCount() == 1) {
            ...
            if (verifyPassword($password, $admin['mot_de_passe'])) {
                // Générer une session
                ...
                $stmt_session->execute([$session_token, $admin['id'], $_SERVER['REMOTE_ADDR'], $_SERVER['HTTP_USER_AGENT'], $expires_at]);
                
                // Mettre à jour la date de dernière connexion
                $query_update = "UPDATE administrateurs SET date_derniere_connexion = NOW() WHERE id = ?";
                $stmt_update = $db->prepare($query_update);
                $stmt_update->execute([$admin['id']]);
                
                // Logger la connexion
                loggerActivite($admin['id'], 'admin', 'CONNEXION', 'Connexion administrateur réussie');
    ...
    error_log("Erreur dans login_admin.php: " . $e->getMessage());
